...
## Skip this part cause IUCN not relevant to the porcupine fisher stuff
...
# do not to get rid of humans - not part of the dataset 

### looking at data only for the year 2009 (sampling sites, species, dates)
## Again, nor applicable to our data
names(data)
...
# camera trap days
camera_days<-cam.days(data,2009.01) # creates a new dataframe with a new 
        #column with the number of days within each unique range of time 
...
events_dd<-event.sp(dtaframe=data, year=2009.01, thresh=1440) # number of minutes in a day
