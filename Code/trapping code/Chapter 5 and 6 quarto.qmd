...
```{r}
 require(reshape)
    hr <- f.separate.events(data, 60)
    sel <- subset(hr, select = c(Sampling.Event, Sampling.Unit.Name, Photo.Date, bin, grp))
    del <- unique(sel)
    dta <- rename(del, c(value = "bin"))
    yrsel <- dta[dta$Sampling.Event == 2009.1, ]
    events <- cast(yrsel, Sampling.Unit.Name ~ value, length)
```

```{r}
events_hh<-event.sp.1(dtaframe=data, year=2009.01, thresh=60) #  thresh in minutes
events_dd<-event.sp.1(dtaframe=data, year=2009.01, thresh=1440) # number of minutes in a day
# creates a matrix with the sampling unit and the presence of species (number of detection)
#giving idea as the number of detections in an hour 
```
...
#f.matrix.creator<-function(data)
	...
	cams<-unique(data$Sampling.Unit.Name) #creates a unique list of camera names 
	cams<-sort(cams) #sorts cameras into ascending numeric order
	rows<-length(cams) #creates a variable thar indicates that there are 
	#start and end dates of sampling periods - number of rows in our matrix
	min<-min(data$Start.Date) #designates a minimum start 
	max<-max(data$End.Date) #designates maximum end
	#giving the maximum total number of days a camera could be active
	cols<-max-min+1 #I think this will be the number of cols in our matrix 
	
	#sampling period
	date.header<-seq(from=min,to=max, by=1) #rows for EVERY SECOND in the span of the 401 days
	mat<-matrix(NA,rows,cols,dimnames=list(cams,as.character(date.header)))
	#investigate the error it may be inconsistent lengths in the names?
