...
```{r}
roads <- read.csv(here("Data","correct_cam_road_sum.csv"))
```
...
```{r}
colnames(roads)[4] <- "road_length_m"
```
...
<<<<<<< HEAD
## Need to re-export the data with the "distance variable attached and figure out what happened with the deployment number not loading.
=======
...
>>>>>>> 74f32d3b4fc8726b0e0aaad32955cad1c0718585
...
```{r}
landuse <- read.csv(here("Data","landuse_fix.cs"))
```
...
porcupine_F_max <- slice(landuse, 689:1032)
porcupine_F_min <- slice(landuse, 1033:1376)
porcupine_M_max <- slice(landuse, 1377:1720)
porcupine_M_min <- slice(landuse, 1721:2064)
```
